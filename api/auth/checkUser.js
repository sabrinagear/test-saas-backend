...
async function routeCheck(req, res, next, userId) {
  ...
  if (req.originalUrl === `/api/group/user/${req.params.id}`) {
    ...
    if (userId !== paramId) {
      return res
        .status(403)
        .json({ warning: `You are not allowed to do that.` });
  ...
  if (
    req.originalUrl === `/api/group/${req.params.id}` &&
    req.method === "GET"
  ) {
    ...
    groupMembersDb
      ...
      .then(members => {
        ...
        if (member.length === 0) {
          return res
            .status(403)
            .json({ warning: `You do not have permissioon to do that.` });
        } else {
          return next();
        }
      })
  ...
  if (
    req.originalUrl === `/api/user/${req.params.id}` &&
    (req.method === "PUT" || "DELETE")
  ) {
    let paramId = Number(req.params.id);
