...
router.post("/", (req, res) => {
  ...
  db.getByUser(group.userID)
    .then(rs => {
      console.log("RS => ", rs);
      if (subType === 1 && rs.length >= 1) {
        return res.status(403).json({
          warning: `You do not have permission to do that. Only premium members can create more than one group.`
        });
            ...
            if (groupId.length >= 1) {
              groupMembersDb
                ...
                .then(mId => {
                  ...
                  return res
                    .status(500)
                    .json({ message: `Could not add member to groupMembers.` });
                })
                .catch(err => {
                  console.log(err);
                  const error = {
...
function fetch_group_mem(id) {
  return groupMembersDb.getByGroup(id).then(value => {
    return value;
  });
}
  ...
  groupMembersDb
    ...
    .then(joinedGroups => {
      ...
      for (let i = 0; i < joinedGroups.length; i++) {
        db.getById(joinedGroups[i].groupID).then(groupProfiles => {
...
// router.get('/user/:id', checkUser, async (req, res) => {
//     const id = req.params.id;

//     let groups = [];
//     let members = [];

//     const groupmembs = await groupMembersDb.returnUserGroups(id);

//     // console.log("GM => ", groupmembs);

//     for (let i = 0; i < groupmembs.length; i++) {
//         const grp = await db.getById(groupmembs[i].groupID)
//         groups.push(grp[0]);
//     }
...
//             let curMembs = []
//             const groupMember = await groupMembersDb.getByGroup(groups[i].id);
...
//     try {
//         const grp = await db.getByUser(id);

//         // console.log("GRP => ", grp);
//         for (let i = 0; i < grp.length; i++) {
//             const member = await groupMembersDb.getByGroup(grp[i].id);
    ...
    .then(status => {
      if (status.length >= 1 || !status) {
        return res.status(200).json({
          message: `Group ${id} successfully updated.`,
          id: Number(id)
        });
      }
...
router.delete("/remove/:groupID/:userID", (req, res) => {
  let userID = req.params.userID;
  let groupID = req.params.groupID;
  groupMembersDb
    .getById(groupID, userID)
    .then(entry => {
      // console.log(entry);
      if (entry && (entry[0].moderator === 1 || entry[0].moderator === true)) {
        db.remove(groupID).then(response => {
          ...
          groupMembersDb.removeGroup(groupID).then(response => {
            ...
            return res
              .status(200)
              .json({ message: `Group successfully deleted.` });
          });
        });
      } else {
        return res
          .status(403)
          .json({ warning: `You are not authorized to do that.` });
      }
    })
    .catch(err => {
      console.log(err);
...
/**************************************************/
router.get("/invite/:userId::groupId", (req, res) => {
