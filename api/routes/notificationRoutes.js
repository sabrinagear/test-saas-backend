...
router.post("/", (req, res) => {
  ...
  db.add(notif)
    .then(id => {
      return res
        .status(200)
        .json({ message: "Notification successfully added.", id: id[0] });
    })
    .catch(err => {
      const error = {
...
router.get("/:id", (req, res) => {
  ...
  db.getById(id)
    .then(notif => {
      ...
      return res
        .status(404)
        .json({ error: `The requested notification does not exist.` });
    })
    .catch(err => {
      const error = {
...
router.get("/", (req, res) => {
  db.get()
    .then(notifs => {
      ...
      return res
        .status(404)
        .json({ message: "The requested notifications do not exist." });
    })
    .catch(err => {
      const error = {
